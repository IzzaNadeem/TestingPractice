...
class MovieAPITests: XCTestCase {
    ...
    func testMovieExists() {
        ...
        //start network request
        MovieAPI.manager.searchMovies(keyword: "") { (error, movieArray) in
            if let error = error {
                //Fail test
                XCTFail("Could not get data: \(error)")

//            } else if let movieArray = movieArray {
//                 XCTAssertEqual(movieArray[0]., "Blue Collar Comedy Tour: One for the Road", "Movie does not exist")
//            }
        }
        //wait 10 seconds for results because it's async
        wait(for: [movieResultsExpectation], timeout: 10)
